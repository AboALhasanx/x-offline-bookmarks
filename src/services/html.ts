...
const ZOOM_SCRIPT = [
  ...
  "window.zoomImg=function(src){img.src=src;scale=1;tx=0;ty=0;apply();document.getElementById('zoomov').classList.add('open');try{window.ReactNativeWebView.postMessage(JSON.stringify({type:'ZOOM_OPEN'}));}catch(e){}};",
  "window.closeZoom=function(){document.getElementById('zoomov').classList.remove('open');try{window.ReactNativeWebView.postMessage(JSON.stringify({type:'ZOOM_CLOSE'}));}catch(e){}};",
